...
import { ArrowLeft, Plus, Save, FilePen, HeartPulse, Activity, AlertTriangle, Baby, Brain, FileText, X, Trash2, ClipboardList } from 'lucide-react'
...
import { inferNombrePartsFromCompleto, formatDateForInput } from '@/lib/estudianteFicha'
...
export default function EditarEstudiantePage() {
  ...
  const [form, setForm] = useState({
    ...
    const handleSwitchActivo = (checked: boolean) => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (cancelled) return
        if (d.error) {
          toast.error(d.error || 'No encontrado')
          router.push('/estudiantes')
          return
        }
        const inferred = inferNombrePartsFromCompleto(d.nombre || '')
        const nombresVal = d.nombres && String(d.nombres).trim() ? String(d.nombres).trim() : inferred.nombres
        const primerVal =
          d.primerApellido && String(d.primerApellido).trim()
            ? String(d.primerApellido).trim()
            : inferred.primerApellido
        const segundoVal =
          d.segundoApellido && String(d.segundoApellido).trim()
            ? String(d.segundoApellido).trim()
            : inferred.segundoApellido
        setForm({
          nombres: nombresVal,
          primerApellido: primerVal,
          segundoApellido: segundoVal,
          nie: d.nie || '',
          lugarNacimiento: d.lugarNacimiento || '',
          fechaNacimiento: formatDateForInput(d.fechaNacimiento),
          correo: d.correo || '',
          religion: d.religion || '',
          estadoCivil: d.estadoCivil || '',
          estudioParvularia: Boolean(d.estudioParvularia),
          repiteGrado: Boolean(d.repiteGrado),
          transporte: d.transporte || '',
          distancia: d.distancia || '',
          tallaPantalon: d.tallaPantalon || '',
          tallaCamisa: d.tallaCamisa || '',
          tallaZapatos: d.tallaZapatos || '',
          direccion: d.direccion || '',
          departamento: d.departamento || '',
          municipio: d.municipio || '',
          canton: d.canton || '',
          viveCon: d.viveCon || '',
          dependeEconomicamente: d.dependeEconomicamente || '',
          miembrosFamilia: d.miembrosFamilia || '',
          estudianteTrabaja: Boolean(d.estudianteTrabaja),
          tieneHijos: Boolean(d.tieneHijos),
          grado: d.grado || '',
          seccion: d.seccion || 'A',
          turno: d.turno || 'Matutino',
          encargado: d.encargado || '',
          telefono: (d.telefono && String(d.telefono).replace(/\D/g, '').slice(0, 8)) || '',
          padreProfesion: d.padreProfesion || '',
          padreTelefonoTrabajo: (d.padreTelefonoTrabajo && String(d.padreTelefonoTrabajo).replace(/\D/g, '').slice(0, 8)) || '',
          padreDireccion: d.padreDireccion || '',
          padreDepartamento: d.padreDepartamento || '',
          padreMunicipio: d.padreMunicipio || '',
          padreCanton: d.padreCanton || '',
          documentosEntregados: Array.isArray(d.documentosEntregados) ? d.documentosEntregados : [],
        ...
        if (!cancelled) toast.error('Error cargando datos')
      } finally {
  ...
  if (pageLoading) {
    return (
      <div className="flex flex-col h-full">
        <Header title="Cargando..." subtitle="Expediente del estudiante" />
  ...
  return (
    <div className="flex flex-col h-full">
      <Header title="Editar estudiante" subtitle="Actualizar expediente y ficha" />
            ...
            <CardHeader className="border-b">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>
                    Los cambios se guardan en el expediente. Todos los campos son opcionales; podés completar el
            ...
            <CardContent className="pt-6">
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="rounded-lg border bg-muted/30 p-4 text-sm text-muted-foreground">
                  <p className="font-medium text-foreground mb-1">Sobre la edición</p>
                  <p className="text-xs">
                    Los cambios actualizan el expediente y la ficha. No se crean talonarios nuevos ni usuarios al guardar.
                  </p>
                </div>
                ...
                <div className="sm:col-span-2 p-3 rounded-lg border bg-muted/30 flex items-center justify-between gap-4">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {form.activo
                        ? 'Activo: se pueden generar talonarios y registrar pagos según las reglas del sistema.'
                        : 'Inactivo: restringe acciones como nuevos talonarios.'}
                    </p>
                  </div>
                  <Switch id="activo" checked={form.activo} onCheckedChange={handleSwitchActivo} />
                </div>
